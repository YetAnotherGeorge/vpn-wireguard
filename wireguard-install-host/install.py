...
def main():
   ...
   ccu.RunCommandContainer(f"ufw delete allow {SERVER_PORT}").Check()
   ccu.RunCommandContainer(f"ufw allow {SERVER_PORT} comment \"Wireguard\"").Check()
   ...
   iface_cmd = ccu.RunCommandContainer("ip route list default", True).Check()
